  ...
  const addTask = () => {
    ...
    setTasks((prev) => [
      ...prev,
      {
        ...
        title: newTask,
        subtasks: subs.map((s) => ({ id: crypto.randomUUID(), text: s, done: false })),
        expanded: true,
      },
    ]);
    setNewTask("");
    ...
    <div className="p-6 md:p-10 max-w-2xl mx-auto space-y-8">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-muted-foreground mt-1">
          Big tasks feel smaller when broken into steps. Add a task and we'll suggest steps.
        </p>
      </motion.div>
      ...
      <div className="flex gap-2">
        <Input
          ...
          placeholder="e.g. Write my history essay"
          className="rounded-xl"
        />
        <Button onClick={addTask} className="rounded-xl gap-1">
          <Sparkles className="w-4 h-4" /> Break Down
        </Button>
      </div>
                        ...
                        {task.subtasks.map((sub) => (
                          <button
                            ...
                            className={`flex items-center gap-3 w-full text-left px-3 py-2 rounded-lg transition-colors ${
                              sub.done ? "bg-sage/30" : "hover:bg-muted/60"
                            }`}
                          >
                            <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors ${
                              sub.done ? "bg-primary border-primary" : "border-border"
                            }`}>
                              {sub.done && <Check className="w-3 h-3 text-primary-foreground" />}
                            </div>
...
const SubtaskAdder = ({ onAdd }: { onAdd: (text: string) => void }) => {
  ...
  return (
    <div className="flex gap-2 pt-1">
      ...
      />
      <Button size="sm" variant="ghost" onClick={() => { onAdd(text); setText(""); }} className="h-8">
